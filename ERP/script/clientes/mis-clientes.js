$(document).ready(function() {
    tabla = "servicios_inv";
    table = $('#clientes-lista').DataTable({
        ...
        ajax: {
            method: "POST",
            url: "../servidor/clientes/traer-lista-clientes.php",
            data: {"tabla" : tabla},
            dataType: "json"
        },
      ...
      columns: [
        ...
        {title: "Correo",      data: "correo"},
        {
          ...
          render: function (row) {
            ...
            return '<div style="display: flex; flex-direction: column;"><button type="button" onclick="editarCliente('+row.id+');" id="'+ row.id +'" '+
            'class="buttonEditar btn btn-warning"><i class="fa fa-edit"></i>'+
            '</button><br><button type="button"  style="margin-top:-18px; margin-bottom:7px"   onclick="borrarCliente('+row.id+');" style="margin: 8px 0px" class="buttonBorrar btn btn-danger">'+
...
function agregarCliente() {
  ...
  Swal.fire({
    ...
    html: '<form class="mt-4" id="agregar-cliente">'+
  ...
  '</div>'+
    ...
    '</div>'+
     ...
     '<div id="app-cover">'+
          '<div class="row">'+
            '<label class="m-auto"><b>Tipo de cliente</b></label>'+
           '<div class="toggle-button-cover">'+
             '<div class="button-cover">'+
              '<div class="button r" id="button-4">'+
               '<input type="checkbox" id="tipo-cliente" class="checkbox">'+
                ' <div class="knobs"></div>'+
                 '<div class="layer"></div>'+
               '</div>'+
            ' </div>'+
           '</div>'+
           '<span id="empresa-label" class="label-empresa mr-5">Empresa</span><span id="persona-label" >Persona</span>'+
           '</div>'+
'</form>',
   ...
   if(result.isConfirmed){
    ...
    $.ajax({
      type: "POST",
      url: "../servidor/clientes/agregar-cliente.php",
      data:datos,
...
function editarCliente(id){
  ...
  $.ajax({
          type: "post",
          url: "../servidor/clientes/traer-cliente.php",
          ...
          '</form>',
              ...
              didOpen: function(){
                ...
                if(response.tipo_cliente == "empresa"){
                  $("#tipo-cliente").prop( "checked", false);
                    tipo_cliente = "empresa";
                }else if(response.tipo_cliente == "persona"){
                  $("#tipo-cliente").prop( "checked", true);
                    tipo_cliente = "persona";
                }
...
function  borrarCliente(id){
      ...
      Swal.fire({
        ...
        html: '<span>El cliente se eliminara de forma permanente.</span>',
        showCancelButton: true,
        ...
        focusConfirm: false }).then((result) => {
            if(result.isConfirmed){
              $.ajax({
                type: "POST",
                url: "../servidor/clientes/borrar-cliente.php",
                data:{  "id":id, "tabla": tabla},
                cache: false,
                success: function(response) {
                  if (response) {
